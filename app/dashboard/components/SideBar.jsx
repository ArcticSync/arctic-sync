  ...
  const SidebarItem = ({ href, icon, label }) => (
    <Link href={href}>
      <div className="group relative inline-block">
        <div className="bg-gray-100 hover:bg-gray-200 cursor-pointer my-4 p-3 rounded-l inline-block transition-opacity">
          {icon}
        </div>
        <div className="opacity-0 invisible group-hover:opacity-100 group-hover:visible absolute top-0 left-1/2 transform -translate-x-1/2 bg-gray-600 text-white p-2 rounded-md mt-2 text-xs">
          {label}
        </div>
      </div>
    </Link>
